...
public enum LoginFailureMessage {
    ...
    USER_NOT_FOUND("존재하지 않는 계정입니다.", false, false),
    BAD_CREDENTIALS("아이디 또는 비밀번호가 올바르지 않습니다.", true, true),
    DISABLED("비활성화된 계정입니다. 관리자에게 문의하세요.", false, true),
    LOCKED("계정이 잠겨 있습니다. 관리자에게 문의하세요.", false, true),
    CREDENTIALS_EXPIRED("비밀번호 유효 기간이 만료되었습니다. 비밀번호를 재설정하세요.", false, true),
    ACCOUNT_EXPIRED("계정 유효 기간이 만료되었습니다.", false, true),
    UNKNOWN("로그인에 실패했습니다. 다시 시도해주세요.", false, true);

    private final String defaultMessage;
    private final boolean recordFailureCnt; // true: 실패 횟수 증가, false: 실패 횟수 증가하지 않음
